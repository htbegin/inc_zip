...
from .core import compare_states, Changes
...
def create_parser():
    ...
    backup_parser.add_argument("-i", "--increments", nargs='*', help="(Optional) Path to one or more existing incremental zips, in order of creation.")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.command == "backup":
        ...
        # For now, we just call it, the mock will return {}
        old_state = get_zip_metadata(args.base_zip)
        if args.increments:
            for inc_zip in args.increments:
                # In reality, we would merge this state
                get_zip_metadata(inc_zip)
